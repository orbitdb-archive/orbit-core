  ...
  const tests = [
    {
      ...
      minInterval: 0,
      content: 'Hello #',
      clients: [{ name: 'daemon1' }, { name: 'daemon2' }, { name: 'daemon3' }, { name: 'daemon4' }]
    },
    {
      ...
      minInterval: 0,
      content: 'Hello world, ',
      clients: [{ name: 'daemon1' }, { name: 'daemon2' }]
    },
    {
    ...
    it(test.description, done => {
      ...
      const createIpfsInstance = c => {
        const repoPath = path.join(
          rootPath,
          c.name,
          '/ipfs/network-stress-test-ipfs-' + new Date().getTime()
        )
        console.log('Starting IPFS instance <<>>', repoPath)
        return startIpfs(
          Object.assign({}, config.defaultIpfsConfig, {
            repo: repoPath,
            start: true
          })
        )
      ...
      const setupAllMessages = clients => {
        ...
        const setupMessages = client =>
          texts.reduce((res, acc) => {
      ...
      const sendAllMessages = () => {
        if (sequential) {
          return pEachSeries(allMessages, e => pEachSeries(e.messages, sendMessage)).then(() =>
            console.log()
          )
        } else {
          return pMap(allMessages, e => pEachSeries(e.messages, sendMessage)).then(() =>
            console.log()
          )
        }
      }
      ...
      const sendMessage = message => {
        return new Promise((resolve, reject) => {
          ...
          if (maxInterval === -1) {
            message.orbit
              .send(channelName, message.content)
              .then(() =>
                process.stdout.write(
                  `\rSent: ${Math.floor(++i / clients.length)} (x${clients.length}) total: ${a}`
                )
              )
              .then(resolve)
              .catch(reject)
                ...
                .then(() =>
                  process.stdout.write(
                    '\rReceived: ' +
                      msgCount.toString() +
                      ' / ' +
                      messageCount * clients.length * clients.length
                  )
                )
      ...
      pMap(
        clientData,
        c => {
          return createIpfsInstance(c).then(ipfs => startOrbitClient(c, ipfs))
        },
        { concurrency: 1 }
      )
        .then(async result => {
          clients = result
          await clients[0]._ipfs.swarm.connect(
            clients[1]._ipfs._peerInfo.multiaddrs._multiaddrs[0].toString()
          )
          await clients[1]._ipfs.swarm.connect(
            clients[0]._ipfs._peerInfo.multiaddrs._multiaddrs[0].toString()
          )
          allChannels = await pMap(clients, orbit => orbit.join(channelName))
          waitForAllPeers = clients.map(orbit =>
            waitForPeers(orbit._ipfs, orbit.getChannel(channelName))
          )
        ...
        .then(result => {
          ...
          result.forEach(messages => {
            assert.strict.equal(messages.length, messageCount * clients.length)
          })

          // Both clients have the same messages in same order
          result.reduce((prev, messages) => {
            assert.strict.deepEqual(messages, prev)
            return messages
          }, result[0])

          // Success
          pEachSeries(clients, orbit => orbit.disconnect()).then(() => done())
        })
        .catch(done)
    })
  })

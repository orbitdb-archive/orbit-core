'use strict'
const IdentityProvider = require('./identityprovider-interface')
  ...
  async _createWallet (options = {}) {
    if (options.mnemonicOpts) {
      ...
      return Wallet.fromMnemonic(
        options.mnemonicOpts.mnemonic,
        options.mnemonicOpts.path,
        options.mnemonicOpts.wordlist
      )
    }
    if (options.encryptedJsonOpts) {
      ...
      return Wallet.fromEncryptedJson(
        options.encryptedJsonOpts.json,
        options.encryptedJsonOpts.password,
        options.encryptedJsonOpts.progressCallback
      )
    }
